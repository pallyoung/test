...
//编码加密
  ...
  window.chirp = function(message, ecc) {
    ...
    oscillator.type = 0;
    gainNode =context.createGain()|| context.createGainNode();
    ...
    for (i = _j = 0, _len1 = chirp.length; _j < _len1; i = ++_j) {
      char = chirp[i];
      oscillator.frequency.setValueAtTime(freqCodes[char], now + (beepLength / 1000 * i));//产生音频 不需要生成音频文件，只需要播放出声音就可以了
    }
